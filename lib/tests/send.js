...
const Send = async (fSendAmount, iTotalTxPerWallet) => {
	...
	for (let iTxEpoch = 1; iTxEpoch <= iTotalTxPerWallet; iTxEpoch++) {
		...
		let arDfdSendChunk = arWallets.map(async (oWallet) => {
			...
			return oMinterHelper.sendCoinTo(oWallet, sSendToAddress, fSendAmount).
					catch((err) => {
		...
		await Promise.all(arDfdSendChunk.map(oUtils.fnReflect)).
				then(results => {
					let
							...
							iResolved  = results.length - arRejected.length;

					iSuccessTxCount += iResolved;

					oLogger.debug(arRejected, ` Ok: (${iResolved} of ${results.length})`);

					if (!bIsDebugMode) {
						let iEpochTime = process.hrtime()[0];

						oCliProgress.update(iTxEpoch, {
							fTxPerSec: iSuccessTxCount / (iEpochTime - iStartTime)
						});
					}

				}).
				catch(() => oLogger.error('Sending batch failed'));
