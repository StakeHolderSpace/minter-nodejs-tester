...
const FundWallets = async (arWallets, arCheckBook) => {
	...
	let arFundedWallets = [];
	let iChunkSize = Math.round(arWallets.length / 12);
		...
		arFundedWallets = arFundedWallets.concat(
				await Promise.all(arDfdRedeemChunk.map(oUtils.fnReflect)).
						then(results => {
							...
							console.log({arRejected:arRejected});
							...
							console.log('RedeemCheck batch failed');
							return [];
						})
		);
...
const Exec = async (_oMinterHelper, oParams) => {
	...
	await FundWallets(arWallets, arCheckBook);
