...
const App = (function() {
	...
	App.prototype.delegateTo = async function(oWallet, iDelegateAmount, sNodePubKey) {
		...
		if ((oWallet instanceof oMinterWallet.default) && sDelegateToNodePubKey.length) {
			const oMinterNode = new oMinterSdk.Minter({apiType: 'node', baseURL: sNodeUrl, timeout: 5000});
			...
			return oMinterNode.postTx(oTxParams).then(async (response) => {
				return response;
			}).catch((err) => {
	...
	App.prototype.sendCoinTo = async function(oWalletFrom, sToAddress, fAmount) {
		...
		if ((oWalletFrom instanceof oMinterWallet.default) && 0 < _fAmount) {
			const oMinterNode = new oMinterSdk.Minter({apiType: 'node', baseURL: sNodeUrl, timeout: 8000});
			...
			return oMinterNode.postTx(oTxParams).then((response) => {
				return {
					from  : sFromAddress,
					to    : _sToAddress,
					amount: _fAmount,
